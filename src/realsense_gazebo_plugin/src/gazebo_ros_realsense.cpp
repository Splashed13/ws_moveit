...
void GazeboRosRealsense::OnNewFrame(const rendering::CameraPtr cam,
  ...
  this->image_map_[camera_id].header.frame_id =
      this->cameraParamsMap_[camera_id].optical_frame;
  this->image_map_[camera_id].header.stamp.sec = current_time.sec;
  this->image_map_[camera_id].header.stamp.nsec = current_time.nsec;
  ...
  fillImage(this->image_map_[camera_id], pixel_format, cam->ImageHeight(),
  ...
  auto camera_info_msg =
      cameraInfo(this->image_map_[camera_id], cameras.at(camera_id)->HFOV().Radian());
  image_pub->publish(this->image_map_[camera_id], camera_info_msg);
}
...
bool GazeboRosRealsense::FillPointCloudHelper(std::string camera_id, sensor_msgs::PointCloud2 &point_cloud_msg,
                                              ...
                                              uint32_t step_arg, void *data_arg)
{
  ...
  for (uint32_t j = 0; j < rows_arg; j++)
  {
    ...
    for (uint32_t i = 0; i < cols_arg; i++, ++iter_x, ++iter_y, ++iter_z, ++iter_rgb)
    {
      ...
      uint8_t *image_src = (uint8_t *)(&(this->image_map_[camera_id].data[0]));

      if (this->image_map_[camera_id].data.size() == rows_arg * cols_arg * 3)
      {
       if (this->image_map_[camera_id].encoding == sensor_msgs::image_encodings::RGB8) {
          iter_rgb[2] = image_src[i * 3 + j * cols_arg * 3 + 0];
          iter_rgb[1] = image_src[i * 3 + j * cols_arg * 3 + 1];
          iter_rgb[0] = image_src[i * 3 + j * cols_arg * 3 + 2];
        }
        else if (this->image_map_[camera_id].encoding == sensor_msgs::image_encodings::BGR8) {
          iter_rgb[0] = image_src[i * 3 + j * cols_arg * 3 + 0];
          iter_rgb[1] = image_src[i * 3 + j * cols_arg * 3 + 1];
          iter_rgb[2] = image_src[i * 3 + j * cols_arg * 3 + 2];
        }
        else {
          throw std::runtime_error("unsupported colour encoding: " + this->image_map_[camera_id].encoding);
        }
      }
      else if (this->image_map_[camera_id].data.size() == rows_arg * cols_arg)
      {
...
void GazeboRosRealsense::OnNewDepthFrame() {
  ...
  if (pointCloud_ && this->pointcloud_pub_.getNumSubscribers() > 0)
  {
    ...
    FillPointCloudHelper(COLOR_CAMERA_NAME, this->pointcloud_msg_, this->depthCam->ImageHeight(),
                         this->depthCam->ImageWidth(), 2 * this->depthCam->ImageWidth(),
